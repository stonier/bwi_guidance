...
MultiThreadedMCTS<StateQRR14, StateQRR14Hash, ActionQRR14>::Params mcts_params_;
...
InstanceResult testInstance(int seed, bwi_mapper::Graph& graph, 
  ...
  for (size_t method = 0; method < methods.size(); ++method) {
    ...
    boost::shared_ptr<ValueIteration<StateQRR14, ActionQRR14> > vi;
    boost::shared_ptr<MultiThreadedMCTS<StateQRR14, StateQRR14Hash, ActionQRR14> > mcts;
    ...
    if (params.type == HEURISTIC) {
      ...
      mcts_params_.rewardBound = params.mcts_reward_bound;
      ...
      mcts.reset(new MultiThreadedMCTS<StateQRR14, StateQRR14Hash, ActionQRR14>(mcts_model_updator,
